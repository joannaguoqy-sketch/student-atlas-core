...
import { Download, Loader2, Plus, User } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "@/hooks/use-toast";
import { exportStudentProfile } from "@/lib/exportHelpers";
import { calcFormativeTotal, calcFinalExamTotal, calcFinalWeighted, calcFinalTotal, getScoreClass } from "@/types";
import type { FormativeScore, FinalExam } from "@/types";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogTrigger } from "@/components/ui/dialog";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";

interface GrowthLogRow { id: string; record_date: string; content: string; type_name: string; tag_names: string[]; }
...
export default function StudentProfilePage() {
  ...
  const fetchData = async () => {
    ...
    setStudent(stu); setSectionInfo(sec);
    ...
    if (sectionId) {
      ...
      setFs(fsData as any); setFe(feData as any);
    }

    const { data: logs } = await supabase.from("growth_logs").select("*, growth_types(display_name)").eq("student_id", id).eq("is_deleted", false).order("record_date", { ascending: false });
    const logRows: GrowthLogRow[] = [];
    for (const log of logs || []) {
      const { data: logTags } = await supabase.from("growth_log_tags").select("tag_id, tags(display_name)").eq("growth_log_id", log.id);
      logRows.push({ id: log.id, record_date: log.record_date, content: log.content, type_name: (log as any).growth_types?.display_name || "", tag_names: (logTags || []).map((t: any) => t.tags?.display_name || "") });
    }
  ...
  const handleAddLog = async () => {
    ...
    if (!termId) { const { data: t } = await supabase.from("terms").select("id").limit(1).single(); termId = t?.id; }
    if (!termId) { toast({ title: "无法确定学期", variant: "destructive" }); setSaving(false); return; }
    const { data: log, error } = await supabase.from("growth_logs").insert({ student_id: id, term_id: termId, course_id: sectionInfo?.course_id || null, class_section_id: sectionId || null, type_id: newLogTypeId, content: newLogContent.trim() }).select("id").single();
    if (error || !log) { toast({ title: "创建失败", description: error?.message, variant: "destructive" }); setSaving(false); return; }
    if (newLogTagIds.length > 0) {
      await supabase.from("growth_log_tags").insert(newLogTagIds.map((tagId) => ({ growth_log_id: log.id, tag_id: tagId })));
    }
    toast({ title: "成长记录已添加" });
    setAddLogOpen(false); setNewLogTypeId(""); setNewLogContent(""); setNewLogTagIds([]); setSaving(false);
    fetchData();
  };

  const toggleTag = (tagId: string) => { setNewLogTagIds((prev) => prev.includes(tagId) ? prev.filter((t) => t !== tagId) : [...prev, tagId]); };
  ...
  const handleExport = async () => {
    ...
    setExporting(true);
    try { await exportStudentProfile(id!, sectionId); toast({ title: "导出成功", description: "文件已开始下载" }); }
    catch (err: any) { toast({ title: "导出失败", description: err.message, variant: "destructive" }); }
    finally { setExporting(false); }
  };

  const formativeItems = fs ? [
    { label: "个人问答", value: fs.qa_score, weight: "5%" }, { label: "小组协作", value: fs.group_score, weight: "5%" },
    { label: "思想素养", value: fs.ideology_score, weight: "5%" }, { label: "口语", value: fs.speaking_test_score, weight: "5%" },
    { label: "听力", value: fs.listening_test_score, weight: "5%" }, { label: "作业", value: fs.homework_score, weight: "10%" },
    { label: "线上任务", value: fs.online_task_score, weight: "15%" },
  ] : [];

  const examItems = fe ? [
    { label: "词汇", value: fe.vocab, max: 10 }, { label: "选词填空", value: fe.cloze, max: 10 },
    { label: "判断正误", value: fe.tf, max: 10 }, { label: "信息匹配", value: fe.match, max: 20 },
    { label: "深度阅读", value: fe.deep, max: 20 }, { label: "翻译", value: fe.translation, max: 15 },
    { label: "写作", value: fe.writing, max: 15 },
  ] : [];

  const tagGroups = tags.reduce((acc: Record<string, any[]>, t: any) => { (acc[t.group_name] = acc[t.group_name] || []).push(t); return acc; }, {});

  return (
    <div>
      <PageHeader title={`${student.name} - ${courseName || "全部"} - ${termName} - ${sectionName}`} actions={
        <div className="flex gap-2">
          <Dialog open={addLogOpen} onOpenChange={(open) => { setAddLogOpen(open); if (open) loadGrowthConfig(); }}>
            <DialogTrigger asChild><Button size="sm" variant="outline" className="gap-1.5"><Plus className="h-4 w-4" /> 新增成长记录</Button></DialogTrigger>
            <DialogContent className="max-w-lg">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <select className="w-full border border-input rounded-md px-3 py-2 text-sm bg-background" value={newLogTypeId} onChange={(e) => setNewLogTypeId(e.target.value)}>
                    ...
                    <div key={group} className="mb-2">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {(groupTags as any[]).map((t) => (
                          <button key={t.id} type="button" className={`text-xs px-2 py-1 rounded transition-colors ${newLogTagIds.includes(t.id) ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`} onClick={() => toggleTag(t.id)}>
                            {t.display_name}
                          </button>
                        ))}
                      </div>
          ...
          <Button size="sm" variant="outline" className="gap-1.5" onClick={handleExport} disabled={exporting}>
            {exporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />} 导出
          </Button>
        </div>
      } />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-4">
        <TabsList>
          <TabsTrigger value="profile">成长档案</TabsTrigger>
          <TabsTrigger value="portrait" onClick={() => navigate(`/students/${id}/portrait${sectionId ? `?section=${sectionId}` : ""}`)}>
            <User className="h-3.5 w-3.5 mr-1" /> 数字画像
          </TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        <div className="bg-card rounded-lg p-5 border border-border">
          ...
          {formativeItems.length === 0 ? <p className="text-sm text-muted-foreground">暂无数据</p> : (
              ...
              {formativeItems.map((item) => (
                <div key={item.label} className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-24 h-2 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-primary" style={{ width: `${item.value}%` }} /></div>
        ...
        <div className="bg-card rounded-lg p-5 border border-border">
          ...
          {examItems.length === 0 ? <p className="text-sm text-muted-foreground">暂无数据</p> : (
            <div className="space-y-2">
              {examItems.map((item) => (
                <div key={item.label} className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">{item.label} <span className="text-xs">(0-{item.max})</span></span>
                  <div className="flex items-center gap-2">
                    <div className="w-24 h-2 rounded-full bg-muted overflow-hidden"><div className="h-full rounded-full bg-primary" style={{ width: `${(item.value / item.max) * 100}%` }} /></div>
      ...
      <div className="bg-primary/5 rounded-lg p-5 border border-primary/20 mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{formativeTotal} + {finalWeighted} = {finalTotal}</p>
        </div>
                ...
                <div className="flex-1">
                  ...
                  {log.tag_names.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {log.tag_names.map((tn, i) => <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-secondary text-secondary-foreground">{tn}</span>)}
                    </div>
                  )}
                </div>
              </div>
