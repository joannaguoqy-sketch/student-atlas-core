...
// Computed fields that must be ignored
...
export function validateStudentImport(rows: ParsedRow[]): ValidationResult {
  ...
  rows.forEach((row, i) => {
    ...
    if (!row.student_code || String(row.student_code).trim() === "") {
      errors.push({ row: lineNum, field: "student_code", reason: "学号不能为空" });
    }
    if (!row.name || String(row.name).trim() === "") {
      errors.push({ row: lineNum, field: "name", reason: "姓名不能为空" });
    }
    const code = String(row.student_code || "");
    if (seen.has(code)) {
      errors.push({ row: lineNum, field: "student_code", reason: `学号 ${code} 重复` });
    }
    seen.add(code);
  });
  ...
  rows.forEach((row, i) => {
    ...
    required.forEach((f) => {
      if (!row[f] || String(row[f]).trim() === "") {
        errors.push({ row: lineNum, field: f, reason: `${f} 不能为空` });
      }
    });
  });
...
export function validateFormativeImport(rows: ParsedRow[]): ValidationResult {
  const errors: ValidationError[] = [];
  const required = ["student_code", "term_name", "course_name", "section_name"];
  const scoreFields = ["qa", "group", "ideology", "speaking", "listening", "homework", "online"];
  const seen = new Set<string>();
  ...
  rows.forEach((row, i) => {
    ...
    if (seen.has(key)) {
      errors.push({ row: lineNum, field: "student_code", reason: "重复主键(学号+学期+课程+教学班)" });
    }
    seen.add(key);
  });
...
export function validateFinalExamImport(rows: ParsedRow[]): ValidationResult {
  const errors: ValidationError[] = [];
  const required = ["student_code", "term_name", "course_name", "section_name"];
  const scoreFields = ["vocab", "cloze", "tf", "match", "deep", "translation", "writing"];
  const seen = new Set<string>();
